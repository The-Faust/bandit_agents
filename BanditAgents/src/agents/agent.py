from typing import Any, Callable, Dict, Iterable, Self, Tuple, Type
...
class Agent:
    ...
    def __init__(
        ...
        context_hyperparameters: Type[(ContextHyperParameters,)] = ContextHyperParameters(),
        solver_hyperparameters: Type[(BaseSolverHyperParameters,)] = SamplingSolverHyperParameters(),
    ) -> None:
        """_summary_
        ...
        self.context = (
            self.contexts_dict[type(context_hyperparameters).__name__](
                actions = actions,
                **context_hyperparameters.__dict__
            )
        )
        self.solver = (
            self.solvers_dict[type(solver_hyperparameters).__name__](
    ...
    def act(self, *args, **kwargs) -> Tuple[ndarray[int64], ndarray[float64]]:
        ...
        float
            _description_
        """
        action_indexes: ndarray[int64] = empty(self.actions_between_fits, dtype=int64)
        ...
        for i in range(self.actions_between_fits):
            ...
            targets[i] = self.context.execute(action_index=action_index, *args, **kwargs)
    ...
    def info(self) -> Dict[str, any]:
        ...
        """
        agent_info = dict(
            context_info=self.context.info(),
            solver_info=self.solver.info()
        )
